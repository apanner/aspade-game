...
import { defaultDeckCountForPlayers } from './deck-config';
...
const COMPUTER_BOTS = [
  { name: 'Rookie Bot', personality: 'conservative' as const },
  { name: 'Strategic Sam', personality: 'smart' as const },
  { name: 'Blitz Bot', personality: 'aggressive' as const },
]

/** Live game with human host + 3 computer opponents — every seat bids solo (no teams). */
export function createGameWithComputers(
  hostName: string,
  hostId: string,
  options: { title?: string; totalRounds?: number; deckCount?: number; maxPlayers?: number } = {}
): Game {
  const gameId = generateGameCode()
  const totalRounds = options.totalRounds ?? 13
  const maxPlayers = options.maxPlayers ?? 4
  const deckCount = options.deckCount ?? defaultDeckCountForPlayers(maxPlayers)
  ...
  for (const bot of COMPUTER_BOTS) {
  ...
  return {
    id: gameId,
    code: gameId,
    title: options.title || `${hostName}'s vs Computer`,
    description: `Live Spades — ${hostName} vs 3 computer players (solo scoring)`,
...
/** Sync cumulative player scores from completed live rounds (engine already scored each round). */
export function syncLiveGameScores(game: Game): void {
  if (!game.roundScores) {
    game.roundScores = {}
  }
  ...
  for (const round of game.rounds) {
    if (round.status === 'completed' && round.scores && Object.keys(round.scores).length > 0) {
      game.roundScores[round.round] = { ...round.scores }
    }
  }
  ...
  for (const roundScores of Object.values(game.roundScores)) {
    for (const [playerId, score] of Object.entries(roundScores)) {
      if (game.players[playerId]) {
        game.players[playerId].score += Number(score)
      }
    }
  }
}
...
export function fixGameDataStructure(game: Game): Game {
  ...
  // Recalculate scores for manual/legacy games only — live engine already scores rounds
  ...
  // Heal games where round advanced in liveState but currentRound was not persisted
  const live = game.liveState as { cardsPerRound?: number; phase?: string } | undefined
  if (
    game.playMode === 'live' &&
    live?.cardsPerRound != null &&
    live.cardsPerRound !== game.currentRound &&
    (live.phase === 'bidding' || live.phase === 'playing')
  ) {
    game.currentRound = live.cardsPerRound
  }
...
export async function touchPlayerLastActiveGame(
  playerName: string,
  gameId: string,
  playerId: string
): Promise<void> {
  const existing = await loadPlayerProfile(playerName)
  if (!existing) return
  await updatePlayerProfile(playerName, {
    lastActiveGame: { gameId: gameId.toUpperCase(), playerId },
  })
}

/** Fast path: check profile hint first, then optional full scan with timeout. */
export async function findPlayerActiveGamesFast(
  playerName: string,
  profile?: PlayerProfile | null,
  options: { fullScanTimeoutMs?: number } = {}
): Promise<Array<{ gameId: string; game: Game; playerId: string; player: Player }>> {
  const timeoutMs = options.fullScanTimeoutMs ?? 3500

  const last = profile?.lastActiveGame
  if (last?.gameId && last?.playerId) {
    try {
      const game = await loadGameFromFile(last.gameId)
      const player = game?.players[last.playerId]
      if (
        game &&
        player &&
        ...
        return [{ gameId: last.gameId.toUpperCase(), game, playerId: last.playerId, player }]
      }
    } catch {
      // stale hint — fall through to timed scan
    }
  } else if ((profile?.loginCount ?? 0) <= 1) {
    // Brand-new player — no game history to scan
    return []
  }

  const fullScan = findPlayerActiveGames(playerName)
  const timedOut = new Promise<Array<{ gameId: string; game: Game; playerId: string; player: Player }>>(
    (resolve) => setTimeout(() => resolve([]), timeoutMs)
  )

  return Promise.race([fullScan, timedOut])
}
...
/** Live card-table game with a full 4-seat table — not legacy manual junk. */
export function isResumableLiveGame(game: Game): boolean {
  if (game.playMode !== 'live') return false
  if (game.status === 'completed' || game.status === 'cancelled') return false
  if (Object.keys(game.players).length !== 4) return false
  return true
}

/** Mid-game only — skip empty lobbies on login resume. */
export function isMidGameLive(game: Game): boolean {
  if (!isResumableLiveGame(game)) return false
  return game.status === 'playing' || game.status === 'bidding' || game.status === 'scoring'
}
...
export function pickBestResumeGame(
  entries: Array<{ gameId: string; game: Game; playerId: string; player: Player }>
): { gameId: string; game: Game; playerId: string; player: Player } | null {
  const midGame = entries.filter((e) => isMidGameLive(e.game))
  if (midGame.length === 0) return null

  midGame.sort((a, b) => {
    const pa = STATUS_PRIORITY[a.game.status] ?? 9
    const pb = STATUS_PRIORITY[b.game.status] ?? 9
    if (pa !== pb) return pa - pb
    return (b.game.lastActivity ?? 0) - (a.game.lastActivity ?? 0)
  })

  return midGame[0]
}

export function filterDashboardLiveGames(
  entries: Array<{ gameId: string; game: Game; playerId: string; player: Player }>,
  max = 3
): Array<{ gameId: string; game: Game; playerId: string; player: Player }> {
  const live = entries.filter((e) => isResumableLiveGame(e.game))
  live.sort((a, b) => {
    const pa = STATUS_PRIORITY[a.game.status] ?? 9
    const pb = STATUS_PRIORITY[b.game.status] ?? 9
    if (pa !== pb) return pa - pb
...
/** One live table at a time — cancel other in-progress live games for this player. */
export async function abandonOtherLiveGamesForPlayer(
  playerName: string,
  keepGameId: string
): Promise<number> {
  const normalizedKeep = keepGameId.toUpperCase()
  const entries = await findPlayerActiveGames(playerName)
  let cancelled = 0

  for (const { gameId, game } of entries) {
    if (gameId.toUpperCase() === normalizedKeep) continue
    if (game.playMode !== 'live') continue
    if (game.status === 'completed' || game.status === 'cancelled') continue
...
export function calculateRoundScores(game: Game, roundNumber: number): void {
  ...
  if (!round || (round.status !== 'completed' && round.status !== 'review' && round.status !== 'reviewing')) {
    return
  }

  if (game.playMode === 'live' && round.scores && Object.keys(round.scores).length > 0) {
  ...
  Object.keys(game.players).forEach((playerId) => {
    ...
    const score = computeLiveRoundScore(bid, tricks)
...
/** Matches @aspade/engine scorer: bid×10 + overtricks, or −bid×10 if missed. */
export function computeLiveRoundScore(bid: number, tricks: number): number {
  if (tricks >= bid) {
